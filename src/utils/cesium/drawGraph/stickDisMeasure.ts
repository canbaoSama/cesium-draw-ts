...
import { CallbackProperty, Cartographic, Color, EllipsoidGeodesic, PolylineGlowMaterialProperty, ScreenSpaceEventHandler, ScreenSpaceEventType } from 'cesium'
...
export default class DrawGraphStickDisMeasure extends DrawGraphBase {
    constructor(viewer: Viewer) {
        ...
        this.drawType = DRAW_GRAPH_MEASURE_MAP.STICK_DIS_MEASURE.key
    }
    ...
    computeLineDis3d(pntList: Array<DrawCartesian3>) {
        ...
        for (let i = 0; i < pntList.length - 1; i++) {
            const startCartographic = Cartographic.fromCartesian(pntList[i])
            const endCartographic = Cartographic.fromCartesian(pntList[i + 1])
            const ellipsoidGeodesic = new EllipsoidGeodesic(startCartographic, endCartographic)
            total += ellipsoidGeodesic.surfaceDistance / 1000
        }
